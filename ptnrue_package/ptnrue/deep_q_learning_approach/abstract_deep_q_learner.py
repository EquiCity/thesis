...
class AbstractDeepQLearner(AbstractQLearner, abc.ABC):
    ...
    def __init__(self, base_graph: ig.Graph, reward: BaseReward, edge_types: List[str], budget: int, episodes: int,
        ...
        self.actions: torch.Tensor = torch.tensor([e.index for e in self.base_graph.es.select(type_in=edge_types, active_eq=1)])
    ...
    def choose_action(self, state: torch.Tensor, epsilon: float) -> torch.Tensor:
        available_actions = [action_idx for action_idx, _ in enumerate(self.actions)
        ...
        if sample > epsilon:
            with torch.no_grad():
                q_values = self.policy_net(state.float())  #.view(1,-1)
                masked_q_values = q_values.masked_select(available_actions_t)
    ...
    def optimize_model(self):
        ...
        non_final_next_states = torch.cat(non_final_list).view(-1, len(batch.state)).T.float()
        state_batch = torch.cat(batch.state, 0).view(-1, len(batch.state)).T.float()
