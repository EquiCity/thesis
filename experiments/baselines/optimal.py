...
def optimal_baseline(g: ig.Graph, census_data: pd.DataFrame, edge_types: List[str],
                     ...
                     com_threshold: float = 15) -> List[Tuple[List[float], List[ig.Edge]]]:
    if groups is None:
        groups = []
    assert budget > 0

    removable_edges = g.es.select(type_in=edge_types)
    possible_combinations = list(it.combinations(removable_edges, budget))

    rewards = - np.ones(len(possible_combinations)) * np.inf

    for i, candidate in enumerate(tqdm(possible_combinations)):
        g_prime = g.copy()
        g_prime.delete_edges(candidate)
        # ig.plot(g_prime)
    ...
    for cand_i in max_reward_candidates_idxs:
        es_idx_list = [e.index for e in possible_combinations[cand_i]]
        rewards_per_removal = []
        for i in range(0,budget):
            g_prime = g.copy()
            g_prime.delete_edges(es_idx_list[0:i+1])
            rewards_per_removal.append(reward_func(g_prime, census_data, groups, metrics))

        optimal_solutions_and_rewards_per_removal.append((rewards_per_removal, g.es[es_idx_list]))
